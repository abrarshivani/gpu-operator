...
# shellcheck disable=SC2016
assert_fails "release mode refuses to derive a version outside a git checkout" \
    bash -c 'cd "$2" && source "$1" && MODE=release resolve_release_provenance' \
...
# shellcheck disable=SC2016
assert_fails "release mode refuses to derive a version when no tag is reachable" \
    bash -c 'cd "$2" && source "$1" && MODE=release resolve_release_provenance' \
    _ "${gen}" "${untagged_checkout}"

git -C "${untagged_checkout}" -c tag.gpgSign=false -c tag.forceSignAnnotated=false tag v9.9.9
derived="$(bash -c 'cd "$2" && source "$1" \
    && MODE=release resolve_release_provenance \
    && printf "%s %s" "${RELEASE_VERSION}" "${LINK_REF}"' _ "${gen}" "${untagged_checkout}")"
assert_eq "v9.9.9 v9.9.9" "${derived}" \
    "on a tagged commit the version and the link ref are both the tag"

git -C "${untagged_checkout}" -c user.name=test -c user.email=test@example.invalid \
    -c commit.gpgsign=false commit -q --allow-empty -m 'past the tag'
# shellcheck disable=SC2016
assert_fails "release mode refuses a commit that is merely descended from a tag" \
    bash -c 'cd "$2" && source "$1" && MODE=release resolve_release_provenance' \
    _ "${gen}" "${untagged_checkout}"

unsafe_tag_checkout="$(temp_path -d)"
git -C "${unsafe_tag_checkout}" -c init.defaultBranch=main init -q
git -C "${unsafe_tag_checkout}" remote add origin https://github.com/NVIDIA/gpu-operator.git
git -C "${unsafe_tag_checkout}" -c user.name=test -c user.email=test@example.invalid \
    -c commit.gpgsign=false commit -q --allow-empty -m 'tagged with markdown syntax'
git -C "${unsafe_tag_checkout}" -c tag.gpgSign=false -c tag.forceSignAnnotated=false tag 'v1.0(beta)'
# shellcheck disable=SC2016
assert_fails "a tag whose name would truncate every markdown link is rejected" \
    bash -c 'cd "$2" && source "$1" && MODE=release resolve_release_provenance' \
    _ "${gen}" "${unsafe_tag_checkout}"
...
bash -c '
    ...
    main release --output "$3"
' _ "${gen}" "${inheritance_checkout}" "${inherited_document}" >/dev/null 2>&1

assert_eq "1" \
    "$(LC_ALL=C grep -c '^NVIDIA GPU Operator v8\.8\.8$' "${inherited_document}")" \
    "release mode names the tag from the checkout, not an inherited RELEASE_VERSION"
assert_eq "/blob/v8.8.8/vendor/" \
    "$(LC_ALL=C grep -o '/blob/[^/]*/vendor/' "${inherited_document}" | LC_ALL=C sort -u)" \
    "every release link uses the tag from the checkout, not an inherited LINK_REF"
assert_eq "0" \
