...
# An empty repository would match every FROM line, so the builder stages' copies
# shellcheck disable=SC2016
...
license_texts_fixture="$(mktemp -d)"
...
assert_eq "0" \
    "$(DOCKERFILE="${bundled_dockerfile}" BUNDLED_COMPONENTS="${bundled_fixture}" \
       ...
       'source "$1"; third_party_bundled_rows nvcr.io/nvidia/distroless/cc > "${BUNDLED_ROWS}"; check_bundled_coverage nvcr.io/nvidia/distroless/cc' _ "${HERE}/generate-third-party-notices.sh" >/dev/null 2>&1; echo $?)" \
    "check_bundled_coverage passes once every copied path has a row"

# shellcheck disable=SC2016
assert_fails "check_bundled_coverage fails when the named licence text is missing" \
    env DOCKERFILE="${bundled_dockerfile}" BUNDLED_COMPONENTS="${bundled_fixture}" \
    LICENSE_TEXTS_DIR="${render}/no-licenses" bash -c \
    'source "$1"; check_bundled_coverage nvcr.io/nvidia/distroless/cc' _ "${HERE}/generate-third-party-notices.sh"
...
rm -f "${flags_dockerfile}"

rm -rf "${license_texts_fixture}"
