...
build_index() {
    ...
    if cut -d, -f4,5 "${INDEX_FILE}" | LC_ALL=C grep -qE '(^|,)unknown(,|$)'; then
    ...
    if LC_ALL=C awk -F, '$3 == "" || $3 ~ /(^| \/ )Unknown( \/ |$)/ { exit 0 } END { exit 1 }' "${INDEX_FILE}"; then
...
location_markup_for() {
    ...
    while IFS= read -r license_file; do
        location_links+="${location_links:+ / }[${license_file##*/}]($(vendored_license_file_url \
...
final_stage_copies() {
    ...
    LC_ALL=C awk -v base_repository="${base_repository}" '
        ...
        in_final_stage && $1 == "COPY" {
            ...
            for (i = first_source; i < NF; i++) print $i "\t" $NF
        }
    ' "${DOCKERFILE}"
}
...
check_bundled_coverage() {
    ...
    [[ -n "${base_repository}" ]] || die "check_bundled_coverage needs the final stage's image repository."
...
emit_sections() {
    ...
    while IFS=$'\t' read -r package version license_identifier module governing_dir relative_license_dir; do
        ...
        while IFS= read -r license_file; do
            ...
            printf '#### %s\n\n' "${license_file##*/}"
            printf '<%s>\n\n' \
                "$(vendored_license_file_url "${license_file}" "${module}" "${relative_license_dir}")"
            printf '%stext\n' "${fence}"
            cat "${license_file}"
            echo
            printf '%s\n' "${fence}"
