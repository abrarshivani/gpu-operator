...
resolve_release_provenance() {
    git rev-parse --git-dir >/dev/null 2>&1 \
        || die "the release subcommand derives the version from the checkout, and this is not a git repository."

    RESOLVED_COMMIT="$(git rev-parse HEAD)" \
        || die "could not resolve HEAD to a commit."

    RELEASE_VERSION="$(git describe --tags --exact-match 2>/dev/null)" \
        || die "${RESOLVED_COMMIT} carries no tag, so there is no release to name." \
               "Check out the release tag, or use the repo subcommand for an untagged tree."
    LINK_REF="${RELEASE_VERSION}"

    # git permits characters in a tag name that markdown reads as link syntax:
    # a tag such as v1.0(beta) truncates every URL at its first ')'.
    [[ "${RELEASE_VERSION}" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] \
        || die "tag name '${RELEASE_VERSION}' cannot be written into a link." \
...
final_stage_copies() {
    ...
    dockerfile_instructions | LC_ALL=C awk -v base_repository="${base_repository}" '
        ...
        # ADD places files in the image just as COPY does.
...
check_apt_pinned_versions() {
    ...
    while IFS=$'\t' read -r component version _ _ _ _ _; do
        ...
        for install_token in ${apt_install_lines}; do
            ...
            # An unpinned package is resolved by apt at build time, so the
            # recorded version cannot be checked against the Dockerfile. Older
            # release tags predate the pin, so this warns rather than aborts.
...
Usage:
  ...
  generate-third-party-notices.sh release --output FILE

  repo      the document for a working tree: no versions, links into vendor/
  release   the artifact for the checked-out release tag. The version, the ref
            the links cite and the repository they cite all come from the
            checkout, so the document cannot name a release it was not built
            from
EOF
    exit 2
}
