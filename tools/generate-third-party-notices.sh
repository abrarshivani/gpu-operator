...
resolve_release_provenance() {
    ...
    # Links cite the commit, not the tag: a tag can be moved afterwards, and
    # several tags can share one commit, so only the SHA is unambiguous.
...
check_no_stale_overrides() {
    ...
    while IFS=$'\t' read -r override_package _ _; do
        ...
        # Regenerating an older release needs the overrides that release wanted,
        # and those look stale against today's index, so this warns rather than
        # aborts. A row unused by every supported tag should still be removed.
