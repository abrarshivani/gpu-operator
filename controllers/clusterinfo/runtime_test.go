...
func TestGetContainerRuntime(t *testing.T) {
	testCases := []struct {
		...
		{
			// The reverse ordering of the case above, and the only one of the pair that pins the
			// skip: with the recognised node first, an unrecognised successor must leave the
			// result alone rather than overwrite it with the empty string.
	...
	// The core client is built before the OpenShift check, so this fires on any cluster. client-go's
	// TLS failure is a bare fmt.Errorf with no sentinel or type to match, so the cause is pinned by
	// comparing against the error the same constructor produces for the same config. That still
	// fails if this path ever returns an error of its own instead of the client's.
