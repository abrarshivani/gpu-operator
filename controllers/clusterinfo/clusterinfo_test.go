...
func TestNew(t *testing.T) {
	testCases := []struct {
		...
		{
			// Unlike the DriverToolkit lookup, a discovery failure aborts construction: the
			// operator would otherwise cache "DRA unsupported" for the process lifetime.
	...
	// Whether New reaches the Proxy endpoint is suspected bug #5, owned by
	// clusterinfo_characterization_test.go, so it is held out of the comparison rather than frozen
	// twice. Cases that could reach it also register a Proxy handler, because the unhandled-path
	// tripwire is a separate check that this exclusion does not reach.
...
// The stored context is what every non-oneshot accessor passes down, and nothing in the returned
// values reveals which context that was. Each subtest below therefore builds an instance whose
// stored context is already spent and asserts the consequence, so substituting a fresh
// context.Background() in any accessor is visible.
func TestClusterInfoAccessorsUseTheStoredContext(t *testing.T) {
	...
	// This accessor returns no error, so the cancelled context shows up only as the absent
	// ImageStream lookup, which the stub's tripwire enforces, and the nil map that follows it.
	...
	// stop GetDRAResourceGVR. The logger carried on that context is the only trace left, and the
	// record below appears only if the accessor passed the stored context down.
