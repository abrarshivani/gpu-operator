...
func TestGetOpenshiftProxySpec(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// "No proxy configured" is a zero-valued spec behind a non-nil pointer, so the
			// nil guard in internal/state/driver.go does not fire for this case.
		...
		{
			// Unlike getOpenshiftVersion, this function does not special-case NotFound: a missing
			// Proxy object is an error that aborts driver reconciliation.
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			...
			// Pins the hard-coded name "cluster" and the cluster scope: no /namespaces/ segment.
