...
func TestGetDRAResourceGVR(t *testing.T) {
	...
	// A CRD declares its own plural, so "deviceclasses" is a name any project may take. This group
	// and the next one are the two halves of "the scan matches on Kind, never on the name".
	...
	testCases := []struct {
		...
		// One version per case, so each of the three below pins a single arm of the
		// v1 > v1beta2 > v1beta1 loop; the cases that make the loop choose come after them.
		{
		...
		{
			// Discovery collapses one resource served at several versions onto the group's preferred
			// version, so the ordered loop sees a single candidate and the extra versions are inert.
		...
		{
			// The same collapse with the group preference reversed: still one candidate, so v1beta1
			// is the only version the loop can return even though the group also serves v1.
		...
		{
			// Two differently named DeviceClass resources are distinct GroupResources, so both survive
			// discovery and the ordered loop picks v1 over the group's own preferred v1beta1.
		...
		{
			// The scan collects every group serving a DeviceClass kind, so an unrelated CRD of the
			// same kind must not displace the real one.
		...
		{
			// The partial-failure path: an unrelated group that cannot be reached must not hide the
			// groups that were discovered successfully.
