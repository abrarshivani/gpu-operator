...
func TestGetDRAResourceGVRCharacterization(t *testing.T) {
	...
	t.Run("a DeviceClass only in an unrelated group is an error, not an unsupported verdict", func(t *testing.T) {
		ctx := testContext(t)
		// Neither cluster serves resource.k8s.io, so both are equally without DRA; only the unrelated
		// CRD's group/version reaching the collected list separates the verdicts below.
