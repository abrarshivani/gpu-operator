...
func TestGetOpenshiftVersion(t *testing.T) {
	testCases := []struct {
		...
		{
			// Only "Completed" is accepted, so an entry whose State the apiserver has not set yet
			// is skipped like a Partial one rather than treated as anything but completed.
		...
		{
			// The empty-string-with-nil-error sentinel is what tells every downstream caller
			// "this is not OpenShift". Conflating it with an error breaks driver reconciliation.
	...
	// is pinned by comparing against the error the same constructor produces for the same config.
	// That still fails if this path ever returns an error of its own instead of the client's.
...
func TestClusterInfoGetOpenshiftVersion(t *testing.T) {
	testCases := []struct {
		...
		{
			// The legitimate non-OpenShift oneshot state, not an error.
